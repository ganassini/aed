...
int main(void)
{
	...
	printf("%lf", maxAverageRatio(classes, 4, 4));
...
double maxAverageRatio(int** classes, int classesSize, int extraStudents) {
    ...
    for ( int i = 0; i < extraStudents; i++ ) {
        ...
        biggest_c_pr = ((classes[0][0] + 1) / (classes[0][1] + 1)) - (classes[0][0] / classes[0][1]);

        for ( int j = 1; j < classesSize; j++ ) {
            diff_c_pr = ((classes[j][0] + 1) / (classes[j][1] + 1)) - (classes[j][0] / classes[j][1]);
